...
module.exports = function (app, passport) {
	...
	app.post('/addItem', function(req, res) {
		...
		if(Number(req.body.type) === 0) {
			...
			tasks = {userid : req.body.id, name : req.body.name, datetime : req.body.datetime || null, 
			...
			tasks = {userid : req.body.id, name : req.body.name, task : req.body.item, 
	...
	app.post('/getItem', function(req,res) {
		mongodb.getItem(req.body.id, function(err, json) {
			if(!err) {
		...
		geoPlaces(req.body.category, req.body.location, function(err, placesObj) {
			if(!err) {
				...
				if(resultArr) {
					...
					var tasks = {userid : req.body.id, name : req.body.name, task : req.body.item, 
	...
	app.post('/updateCoordinates', function(req, res) {
		console.log("updating coordiantes .. " + req.body.currentLocation.latitude);
		mongodb.getItem(req.body.id, function(err, json) {
			if(!err) {
				if(json && json.length > 0) {
					findNearbyTargetPoints(req.body.id, req.body.currentLocation, function(err, resultStr) {
						if(!(err) && resultStr) {
				...
				if(resultArr && resultArr.length > 0) {
					...
					mongodb.updateCoordiantes(userid, resultArr[0]['_id'],
						function(err, resultStr) {
							if(!err) {
								console.log("eror nearby target points : " + resultStr);
							}
						});
