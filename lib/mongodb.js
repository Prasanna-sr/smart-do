...
module.exports = {
    ...
    deleteItem: function(userid, item, callback) {
        db.collection('usertasks');
        db.bind('usertasks');
        db.usertasks.remove({userid : userid, task : item}, function(err, result) {
            if(!err) {
    ...
    findNearbyTargetPoints: function(userid, coordinates, callback) {
        ...
        db.usertasks.find({userid : userid, active : 1, notified : {$lt : datetime - 3600000}, 
           targetLocation : {"$within" : {"$center" : 
            [[parseFloat(coordinates['longitude']), parseFloat(coordinates['latitude'])], (0.1/69)] }}}, 
            function(err, cursor) {
            if(!err) {
                cursor.toArray(callback);
            }
    ...
    findNearByPlaces: function(userid, coordinates, callback) {
        ...
        db.usertasks.find({userid : userid, active : 1, notified : {$lt : datetime - 3600000}, 
            ...
            [[parseFloat(coordinates['longitude']), parseFloat(coordinates['latitude'])], 1] }}}, 
