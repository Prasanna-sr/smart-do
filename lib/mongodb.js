...
module.exports = {
    ...
    updateCoordiantes :function(userid, myLatLng, callback) {
        db.collection('usertasks');
        db.bind('usertasks');
        db.usertasks.update({userid : userid}, function(err, cursor) {
            if(!err) {
                cursor.toArray(callback);
            }
        });
    },
    nearbyCoordinates : function(callback) {
        bd.collection('usertasks');
        db.bind('usertasks');
